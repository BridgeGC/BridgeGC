...
JRT_ENTRY(void, Runtime1::new_keep_instance(JavaThread* current, Klass* klass))
    NOT_PRODUCT(_new_instance_slowcase_cnt++;)
...
JRT_ENTRY(void, Runtime1::new_type_array(JavaThread* current, Klass* klass, jint length))
  ...
  oop obj = oopFactory::new_typeArray(false, elt_type, length, CHECK);
  current->set_vm_result(obj);
...
JRT_ENTRY(void, Runtime1::new_type_keep_array(JavaThread* current, Klass* klass, jint length))
    NOT_PRODUCT(_new_type_array_slowcase_cnt++;)
    // Note: no handle for klass needed since they are not used
    //       anymore after new_typeArray() and no GC can happen before.
    //       (This may have to change if this code changes!)
    ...
    oop obj = oopFactory::new_typeArray(true, elt_type, length, CHECK);
    current->set_vm_result(obj);
    // This is pretty rare but this runtime patch is stressful to deoptimization
    // if we deoptimize here so force a deopt to stress the path.
    if (DeoptimizeALot) {
...
JRT_ENTRY(void, Runtime1::new_object_array(JavaThread* current, Klass* array_klass, jint length))
  ...
  objArrayOop obj = oopFactory::new_objArray(elem_klass, length, false, CHECK);
  current->set_vm_result(obj);
  // This is pretty rare but this runtime patch is stressful to deoptimization
  // if we deoptimize here so force a deopt to stress the path.
  if (DeoptimizeALot) {
...
JRT_ENTRY(void, Runtime1::new_object_keep_array(JavaThread* current, Klass* array_klass, jint length))
    NOT_PRODUCT(_new_object_array_slowcase_cnt++;)
    ...
    Klass* elem_klass = ObjArrayKlass::cast(array_klass)->element_klass();
    objArrayOop obj = oopFactory::new_objArray(elem_klass, length, true, CHECK);
    current->set_vm_result(obj);
    // This is pretty rare but this runtime patch is stressful to deoptimization
    // if we deoptimize here so force a deopt to stress the path.
    if (DeoptimizeALot) {
