...
OopStorage* OopStorageSet::create_strong(const char* name, bool type, MEMFLAGS memflags) {
  static uint registered_strong = 0;
  assert(registered_strong < strong_count, "More registered strong storages than slots");
  OopStorage* storage = new (memflags) OopStorage(name, type, memflags);
  _storages[strong_start + registered_strong++] = storage;
  return storage;
}

OopStorage* OopStorageSet::recreate_strong(const char* name, bool type, MEMFLAGS memflags) {
    OopStorage* storage = new (memflags) OopStorage(name, type, memflags);
    _storages[strong_start + 4] = storage;
    return storage;
}

OopStorage* OopStorageSet::create_weak(const char* name, bool type, MEMFLAGS memflags) {
