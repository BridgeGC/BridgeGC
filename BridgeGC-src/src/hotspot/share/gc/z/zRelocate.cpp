...
private:
    ...
    ZRelocateSmallAllocator        _small_keep_allocator;
    ZRelocateMediumAllocator       _medium_allocator;
    ZRelocateMediumAllocator       _medium_keep_allocator;

    static bool is_small(ZForwarding* forwarding) {
        return forwarding->type() == ZPageTypeSmall;
    }

    static bool is_keep(ZForwarding* forwarding) {
        return forwarding->keep();
    }
...
public:
    ZRelocateTask(ZRelocationSet* relocation_set) :
            ...
            _small_allocator(),
            _small_keep_allocator(),
            _medium_allocator(),
            _medium_keep_allocator(){}
    ...
    virtual void work() {
        ...
        ZRelocateClosure<ZRelocateSmallAllocator> small_keep(&_small_keep_allocator);
        ZRelocateClosure<ZRelocateMediumAllocator> medium(&_medium_allocator);
        ZRelocateClosure<ZRelocateMediumAllocator> medium_keep(&_medium_keep_allocator);

        for (ZForwarding* forwarding; _iter.next(&forwarding);) {
            if (is_small(forwarding)) {
                if(is_keep(forwarding))
                    small_keep.do_forwarding(forwarding);
                else
                    small.do_forwarding(forwarding);
            } else {
                if(is_keep(forwarding))
                    medium_keep.do_forwarding(forwarding);
                else
